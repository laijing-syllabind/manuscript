...
// ---------------------------------------------------------------------------
// JazzBubble scene — ported from the standalone exercise (bubble/three-scene.js).
//
// Adaptations for living inside the Manuscript "Play room":
//   • sizes to a CONTAINER element, not window (ResizeObserver, not resize).
//   • mouse input is scoped to the container; pointer NDC is rect-relative.
//   • the global keydown/keyup listeners are gated behind isActive() so typing
//     while the Read room is showing never triggers notes.
//   • exposes start()/stop()/dispose() so the React host can pause the loop and
//     fully tear down on unmount.
//   • emissive cycles indigo↔gold (brand) instead of a full-spectrum hue sweep.
// ---------------------------------------------------------------------------
...
/**
 * Initialise the JazzBubble scene.
 *
 * @param {object} opts
 * @param {HTMLCanvasElement} opts.threeCanvas  canvas the WebGL renderer draws into
 * @param {HTMLCanvasElement} opts.shaderCanvas canvas holding the FlowingGradient (used as bg texture)
 * @param {HTMLElement}       opts.container    sizing reference (the mount div)
 * @param {object}            opts.config       { gradient, glass, sphere, grid } — see config.js
 * @param {() => boolean}     opts.isActive     whether the Play room is currently showing
 * @param {boolean}           opts.reduceMotion honour prefers-reduced-motion (no auto-spin)
 * @returns {{ start():void, stop():void, dispose():void }}
 */
export function initThreeScene({ threeCanvas, shaderCanvas, container, config, isActive = () => true, reduceMotion = false }) {
  const { glass: glassConfig, sphere: sphereConfig, grid: gridConfig } = config;
  ...
  const camera = new THREE.PerspectiveCamera(75, VW / VH, 0.1, 100);
  ...
  // Brand emissive cycle endpoints (indigo ↔ gold) instead of full-spectrum hue.
  const emissiveLo = new THREE.Color(glassConfig.emissive);
  const emissiveHi = new THREE.Color(gridConfig.color2 || '#86632C');
  ...
  function makeGrid() {
    ...
    const res = new THREE.Vector2(VW, VH);
    const centerSegs = [];
    const regularSegs = [];
  ...
  // ---- Per-frame syncs ----
  // These re-read the config objects every frame so the Tweakpane mixer's live
  // edits take effect without rebuilding the scene. Structural changes (sphere
  // geometry, grid size/colors) are detected and rebuilt only when they change.
  function syncMaterial() {
    ...
    // Brand emissive cycle: lerp indigo (glass.emissive) ↔ gold (grid.color2),
    // reading both from config so mixer edits to either endpoint apply live.
    const tt = (performance.now() - startTime) / 1000;
    const mix = 0.5 + 0.5 * Math.sin(tt * 0.6);
    emissiveLo.set(glassConfig.emissive);
    emissiveHi.set(gridConfig.color2 || '#86632C');
    material.emissive.copy(emissiveLo).lerp(emissiveHi, mix);
  ...
  function resize() {
    ...
    renderer.setSize(VW, VH, false);
    gridMaterials.forEach((m) => m.resolution.set(VW, VH));
  }
  const ro = new ResizeObserver(resize);
  ro.observe(container);

  // ---- Listeners ----
  const onMouseUp = () => stopNote('_mouse');
  const enableAudioOnce = () => ensureAudio();
  container.addEventListener('mousemove', handleMousemove);
  container.addEventListener('mousedown', handleMousedown);
  container.addEventListener('click', enableAudioOnce, { once: true });
  ...
  // First paint so the room isn't blank before the loop starts.
  resize();
  frame();
  ...
  function dispose() {
    ...
    container.removeEventListener('click', enableAudioOnce);
    ...
    clearInterval(activeNotes['_arrowTimer']);
    Object.keys(activeNotes).forEach((k) => { if (k !== '_arrowTimer') keyUp(activeNotes[k]); });
    scene.remove(grid);
    grid.traverse((obj) => {
      if (obj.geometry) obj.geometry.dispose();
      if (obj.material) obj.material.dispose();
